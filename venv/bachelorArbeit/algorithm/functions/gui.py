...
def add_image():
    ...
    image = Image.open(file_path)   # This line uses the Python Imaging Library (PIL), also known as Pillow, to open the selected image file using the file path stored in file_path. The image is then loaded into the image variable.
    width, height = int(image.width / 2), int(image.height / 2)     # Here, the code calculates new dimensions for the image. It reduces the width and height of the image to half its original size. This will be used later to resize the image.
    image = image.resize((width, height), Image.ANTIALIAS)      # This line resizes the image to the new width and height calculated in the previous step. The Image.ANTIALIAS filter is used for a smoother, high-quality resizing.
    canvas.config(width=image.width, height=image.height)       # This line updates the dimensions of a Tkinter canvas widget (canvas) to match the dimensions of the resized image. It sets the canvas width and height to match the resized image's width and height.
    image = ImageTk.PhotoImage(image)       # Here, the resized image is converted into a Tkinter PhotoImage object using the ImageTk.PhotoImage function. This allows you to display the image in a Tkinter canvas.
